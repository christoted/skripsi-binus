    ...
    fun getAdapter(): GenericAdapter<Payment> {
        ...
        adapter.expressionViewHolderBinding = { item, viewBinding, _ ->
            ...
            with(view) {
                ...
                tvDate.text =
                    DateHelper.getFormattedDateTime(DateHelper.DMY, item.paymentDeadline!!)
            }
            when (taskType) {
                PrPaymentVariantViewHolder.TYPE_PAID -> {
                    view.tvDateTitle.text = ("Terbayar pada")
                    view.viewIndicator.setBackgroundColor(
                        ResourcesCompat.getColor(
                            App.resourses!!,
                            R.color.payment_complete,
                            null
                        )
                    )
                }
